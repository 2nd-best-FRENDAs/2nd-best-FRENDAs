...
def tab1_upload_model():
    ...
    """
    # user input: choose between "antimony" and "sbml"
...
def tab2_solve_model():
    ...
    """    
    # user input: type in time values
    ...
    # Initializing variables that can be changed 
    ...
    if (st.session_state.t0 != t0 or st.session_state.tf != tf or st.session_state.steps != steps):
        st.session_state.solve_button = False
        #WHY IS THE DF CLEARING NOT WORKING??????????
        df = None
...
def tab4_plot_selected():
    ...
    if "df" in st.session_state:
        df = st.session_state.df
        # user input: dropdown menu to choose X-axis 
        ...
        # user input: dropdown menu to choose y species 
